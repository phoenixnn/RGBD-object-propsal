function [fgLogPL, bgLogPL] = m_Unary_LogPL_3D(examples, fgGMMs, bgGMMs, ...
...
fgLogPL = fgLogPL + bgLogPL_U + inc*mask_U;
bgLogPL = bgLogPL + fgLogPL_U + inc*mask_U;
...
for i = 1 : K
   if GMMs.wt(i) ~= 0
      PL(:, i) = exp( -m_GM_logPL_3D(examples, GMMs.mu(:, i), GMMs.detcov(i), GMMs.icov(:,:,i)) );
   end
end
