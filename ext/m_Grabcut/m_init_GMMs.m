function [fgGMMs, bgGMMs, flag] = m_init_GMMs(fgExamples, bgExamples, K)
% INPUTS:
% fgExamples: Nx3
% bgExamples: Mx3
...
% flag: true indicates error
flag = false;
[num_fg, width] = size(fgExamples);
num_bg = size(bgExamples, 1);

% initialize models
fgGMMs.mu = zeros(width, K);
fgGMMs.icov = zeros(width, width, K);
fgGMMs.detcov = zeros(K, 1);
fgGMMs.wt = zeros(K, 1);

bgGMMs.mu = zeros(width, K);
bgGMMs.icov = zeros(width, width, K);
