%%%%MatlabWrapperFromRoboticsVisionMatlabToolboxes
...
function [L_,M_] = igraphseg(im, pts, K, mn, sigma)
    if size(im, 3) ~= 3
    ...
    if nargin < 4
        error('MVTB:igraphseg:badarg', 'must specify image, pts, K, min');
    end  
    ...
    [L,M] = graphseg(im, pts, K, mn, sigma);
